...
public:
  AppDelegatePrivate(AppDelegate *parent) : q_ptr(parent) { lpc.set_order(13); }
  ~AppDelegatePrivate() {}
...
AppDelegate::AppDelegate(QObject *parent)
    : QObject(parent), d_ptr(new AppDelegatePrivate(this)) {
...
void AppDelegate::initAudioSystem() {
  ...
  CircularSeries *circularSeries = chartm->circularSeries();
  SpectrogramSeries *spectrogramSeries = chartm->spectrogramSeries();

  connect(audioRecorder, &AudioRecorder::onBufferReady, this,
          [&, circularSeries, spectrogramSeries](float *buffer, ulong size) {
            Q_D(AppDelegate);
            ...
            double error;
            std::tie(error, coeff, reflections) =
                d->lpc.compute(std::begin(input), std::end(input));
  ...
  connect(audioRecorder, &AudioRecorder::sampleRateChanged, this,
          [circularSeries, spectrogramSeries, audioRecorder](double sr) {
            circularSeries->setSize(static_cast<int>(10 * sr));
            spectrogramSeries->setSampleRate(sr);
            spectrogramSeries->setSize(
